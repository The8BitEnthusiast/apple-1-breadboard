...
; Ported to Ben Eater's hardware configuration by reddit user
; @The8BitEnthusiast

; Define to get Ben Eater's hardware version, otherwise generates original Apple II
; code.
; EATER = 1

; Definitions for 16C550 UART
; ACE_DATA  = $A000   ; Serial data register 
; ACE_LSR   = $A005   ; Line status register

; Definitions for BE6502 65C51 UART
; ACIA_DATA       = $5000
; ACIA_STATUS     = $5001

;CHRIN = $8001           ; temporary definition
;DELAY = $01          ; serial transmission delay - set to 1 when running with slow clock
                     ; or when debugging with emulator, or $FF when running with 1Mhz clock


; Define if you want the Apple 1 Woz Monitor included in the build. Only
; works if start address is $F400.
; --> @The8BitEnthusiast: disabled since Ben's build includes its own version
;WOZMON = 1
...
NEXTOP: DEC A1H ; NO, TRY NEXT OPCODE
  ...
  BEQ TRYNEXT ; NO.
ERR: LDY A2_YSAV ; YES, UNRECOGNIZED INST.
ERR2: TYA
  TAX
...
; The8BitEnthusiast: additional bytes
; to make sure the monitor start address
; remains where it should after Ben Eater's
; hardware conversion
...
; RESTORE = $FF3F
;  .ORG $F689
SW16: JSR A2_SAVE ; PRESERVE 6502 REG CONTENTS
  PLA
...
RTNZ: PLA ; POP RETURN ADDRESS
  PLA
  JSR A2_RESTORE ; RESTORE 6502 REG CONTENTS
...
; The8BitEnthusiast: additional 13 bytes
...
RELADR: JSR PCADJ3
  ...
  INY
PRNTYX: TYA
PRNTAX: JSR A2_PRBYTE ; OUTPUT TARGET ADR
...
MNEMR: .BYTE $D8,$62,$5A,$48,$26,$62
  ...
  .BYTE $A2,$C8 ; (E) FORMAT
  .BYTE $FF,$FF,$FF
A2_STEP: JSR INSTDSP ; DISASSEMBLE ONE INST
  PLA ; AT (PCL,H)
...
BRANCH: CLC ; BRANCH TAKEN,
  ...
  SEC
  BCS PCINC2
NBRNCH: JSR A2_SAVE ; NORMAL RETURN AFTER
  SEC ; XEQ USER OF
...
A2_DIV: LDY #$10 ; INDEX FOR 16 BITS
DIV2: ASL ACL
  ROL ACH
  ROL XTNDL ; XTND/AUX
  ROL XTNDH ; TO AC.
...
HOME: LDA WNDTOP ; INIT CURSOR V
  ...
  BEQ CLEOP1
A2_CR: LDA #$00 ; CURSOR TO LEFT OF INDEX
  STA CH ; (RET CURSOR H=0)
A2_LF: INC CV ; INCR CURSOR V(DOWN 1 LINE)
  LDA CV
...
CAPTST: CMP #$E0
  ...
  AND #$DF
ADDINP: STA A2_IN,X ; ADD TO INPUT BUF
  CMP #$8D
  BNE A2_NOTCR
...
@ADD: ADC A1L
  ...
  PLA
A2_PRBYTE: PHA ; PRINT BYTE AS 2 HEX
  ...
  JSR PRHEXZ
  PLA
A2_PRHEX: AND #$0F ; PRINT HEX DIG IN A-REG
PRHEXZ: ORA #$B0 ; LSB'S
  CMP #$BA
  BCC COUT
  ADC #$06
...
STOR: STA A2_MODE ; KEEP IN STORE MODE
  LDA A2L
  ...
  INC A3H
RTS5: RTS
A2_SETMODE: LDY A2_YSAV ; SAVE CONVERTED ':', '+',
...
VFYOK: JSR NXTA4
  BCC VFY
  RTS
A2_LIST: JSR A1PC ; MOVE A1 (2 BYTES) TO
...
A2_USR: JMP USRADR ; TO USR SUBR AT USRADR
WRITE: LDA #$40
...
CRMON: JSR BL1 ; HANDLE A CR AS BLANK
  ...
  PLA ; AND RTN TO MON
  BNE MONZ
A2_READ: JSR RD2BIT ; FIND TAPEIN EDGE
  LDA #$16
...
BELL: LDA #$87 ; OUTPUT BELL AND RETURN
  JMP COUT
A2_RESTORE: LDA STATUS ; RESTORE 6502 REG CONTENTS
...
RESTR1: LDX XREG
  ...
  PLP
  RTS
A2_SAVE: STA ACC ; SAVE 6502 REG CONTENTS
SAV1: STX XREG
  STY YREG
  PHP
  PLA
  ...
  RTS
.ifdef EATER
A2_RESET: JSR L8 ; SET SCREEN MODE
.else
A2_RESET: JSR SETNORM ; SET SCREEN MODE
.endif
